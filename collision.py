...
class Collision:
    ...
    def RectangleCollidedWithCircle1(circle, rect):
        ...
        distX = abs(circle.pos.x - rect.pos.x-rw2)
        distY = abs(circle.pos.y - rect.pos.y-rh2)

        if distX > (rh2 + circle.radius): return False
        if distY > (rh2 + circle.radius): return False

        if distX <= rh2: return True
        if distY <= rh2: return True
    ...
    def RectangleCollidedWithRectangle(r1, r2):
        """
        Identifica colisão entre Retângulos.
        @param r1: Retângulo 1
        @param r2: Retângulo 2
        @return: True se colidiram, caso contrário False
        """
        # testa se houve colisão
        return (r1.pos.x + r1.width > r2.pos.x) and (r1.pos.x < r2.pos.x + r2.width) and (r1.pos.y + r1.height > r2.pos.y) and (r1.pos.y < r2.pos.y + r2.height)
